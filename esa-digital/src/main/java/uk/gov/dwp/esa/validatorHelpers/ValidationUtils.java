...
public final class ValidationUtils {
	...
	private static boolean isValidCalenderDate(String dateToValidate) {
		...
		return isDateValidInFormat(DATE_FORMAT_1, dateToValidate) || isDateValidInFormat(DATE_FORMAT_2, dateToValidate);
	}
	 ...
	 * 
	 * day may be entered as ‘dd’ or ‘d’ (e.g. 08 or 8) month may be entered as
	 * ‘mm’ or ‘m’ (e.g. 04 or 4) year must be entered as ‘yyyy’ (e.g. 2014)
	 * 
	...
	public static boolean isDateValid(String day, String month, String year) {
		if (ValidationUtils.isEmpty(day) || ValidationUtils.isEmpty(month) || ValidationUtils.isEmpty(year)) {
			return false;
		}
	...
	/**
	 * First checks if the 3 string parameters are a valid date and then checks
	 * if it is sixteen years ago or more. If both conditions are satisfied then
	 * returns true.
	 * 
	 * @param day
	 * @param month
	...
	public static boolean isDateBetweenSixWeeksAgoAndToday(String day, String month, String year) {
		...
		try {
			...
			if (startDate.before(date) && startDate.after(dateSixWeeksAgo)) {
				return true;
			}
		} catch (java.text.ParseException e) {
			return false;
	...
	/**
	 * This convenience method brings together the logic for validating a text
	 * field's maximum length and reporting and logging the error if it exceeds
	 * the max length. It saves repeating the same boilerplate logic multiple
	 * times in the validators.
	 *
	 * @param fieldValueToCheck
	 *            This is the String field value to check, which could be null
	 * @param maxLength
	 *            This specifies the maximum allowed length to validate against
	 * @param fieldIdentifier
	 *            Identifies the field for the validation message on failed
	 *            validation
	 * @param validationCode
	 *            Identifies the validation error code to report (by adding it
	 *            to) the errors list) on failed validation
	 * @param logMessage
	 *            Provides a log message to log on failed validation
	 * @param errors
	 *            The list of validation errors to which any detected here are
	 *            to be added
	 */
	public static void validateFieldMaxLength(String fieldValueToCheck, int maxLength, String fieldIdentifier,
			String validationCode, String logMessage, List<ValidationError> errors) {

		// Setup a list of just one FieldInfo for the value to check and log
		// message
		List<FieldInfo> fieldInfoToCheck = new ArrayList<>();
		fieldInfoToCheck.add(new FieldInfo(fieldValueToCheck, logMessage));

		// Now delegate the call to the multiple field version, giving it just
		// the one field
		validateMultiFieldMaxLength(fieldInfoToCheck, maxLength, fieldIdentifier, validationCode, errors);
	}

	/**
	 * This convenience method brings together the logic for validating the
	 * maximum lengths of a set of test fields that are treated as a group for
	 * validation and reporting (against a single field for all of them) and
	 * logging the error if any of them exceeds the maximum length. This is
	 * applicable to groups of address lines, where each line has the same
	 * maximum length but any errors are displayed against the address as a
	 * whole. This method saves repeating the same boilerplate logic multiple
	 * times across the validators. It makes use of the FieldInfo class that is
	 * nested within this class.
	 *
	 ...
	 * @param errors
	 */
	public static void validateMultiFieldMaxLength(List<FieldInfo> fieldInfoToCheck, int maxLength,
			String fieldIdentifier, String validationCode, List<ValidationError> errors) {

		// Fail if we've been called with an empty list as that's a logic error
		if (fieldInfoToCheck.size() < 1) {
			throw new IllegalArgumentException("No field information to check has been provided (empty list)");
		}

		// Now do the validation field by field and stop if we find a validation
		// error
		for (FieldInfo fieldInfo : fieldInfoToCheck) {
			...
			if (fieldValueToCheck != null && ValidationUtils.isStringLengthOver(fieldValueToCheck, maxLength)) {
